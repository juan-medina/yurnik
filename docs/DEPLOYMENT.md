...
Data eviction (cleaning up unconfirmed journeys older than 30 days, old notifications older than 60 days) is handled by a standalone maintenance binary (`yurnik-maintenance`). It runs daily via a systemd timer on the VPS.
